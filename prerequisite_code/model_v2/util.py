...
def copy_checkpoint(source, target):
  for ext in (".index", ".data-00000-of-00001"):    #注意此处文件命名
...
def load_char_dict(char_vocab_path):    #这个函数可能是不需要的
  vocab = [u"<unk>"]
...
def shape(x, dim):   #获得某一维度的大小
  return x.get_shape()[dim].value or tf.shape(x)[dim]

def ffnn(inputs, num_hidden_layers, hidden_size, output_size, dropout, output_weights_initializer=None):  #前馈神经网络
  if len(inputs.get_shape()) > 4:
    raise ValueError("FFNN with rank {} not supported".format(len(inputs.get_shape())))

  if len(inputs.get_shape()) == 4:
    batch_size = shape(inputs, 0)
    num_sentences = shape(inputs, 1)
    seqlen = shape(inputs, 2)
    emb_size = shape(inputs, 3)
    current_inputs = tf.reshape(inputs, [batch_size*num_sentences * seqlen, emb_size])
  ...
  outputs = tf.nn.xw_plus_b(current_inputs, output_weights, output_bias)   #两个矩阵相乘加偏置

  if len(inputs.get_shape()) == 4:
    outputs = tf.reshape(outputs, [batch_size, num_sentences, seqlen, output_size])
...
def cnn(inputs, filter_sizes, num_filters):
  ...
  for i, filter_size in enumerate(filter_sizes):
    ...
    pooled = tf.reduce_max(h, 1) # [num_words, num_filters]    #求最大值
...
def bach_gather_with_equal_shape(emb, indices):
  #emb和indices的shape相同
  emb = tf.unstack(emb, axis=0)
  indices = tf.unstack(indices, axis=0)
  gathered = [tf.gather(x, y) for x, y in zip(emb, indices)]
  return tf.stack(gathered, axis=0)

class RetrievalEvaluator(object):    #该函数需要修改
  def __init__(self):
    self._num_correct = 0
    self._num_gold = 0
...
class EmbeddingDictionary(object):
  ...
  def load_embedding_dict(self, path):
    ...
    if len(path) > 0:
      vocab_size = None
      with open(path) as f:        #构建一个词表并返回，句子 embedding
