...
class CorefModel(object):
  ...
  def get_predictions_and_loss(self, pre_topic_word, post_topic_word, pre_topic_start_index, pre_topic_end_index, post_topic_start_index, post_topic_end_index, pre_tokens, post_tokens, pre_context_word_emb, post_context_word_emb, pre_head_word_emb, post_head_word_emb, pre_lm_emb, post_lm_emb, pre_char_index, post_char_index, pre_text_len, post_text_len, is_training, label):
    ...
    # pre_topic作为后序词    pre_antecedent
    pre_topic_relation_score += self.get_topic_relation_score_equal(pre_topic_pre_antecedent_cosin, pre_top_antecedent_scores, post_topic_pre_span_cosin, pre_top_span_mention_scores, pre_k, pre_c)
    # pre topic作为后序词    post antecedent
    pre_topic_relation_score += self.get_topic_relation_score_equal(pre_topic_post_antecedent_cosin, post_top_antecedent_scores, post_topic_post_span_cosin, post_top_span_mention_scores, post_k, post_c)
    # post topic作为后序词    pre antecedent
    post_topic_relation_score += self.get_topic_relation_score_equal(post_topic_pre_antecedent_cosin, pre_top_antecedent_scores, pre_topic_pre_span_cosin, pre_top_span_mention_scores, pre_k, pre_c)
    # post topic作为后序词     post antecedent
    post_topic_relation_score += self.get_topic_relation_score_equal(post_topic_post_antecedent_cosin, post_top_antecedent_scores, pre_topic_post_span_cosin, post_top_span_mention_scores, post_k, post_c)
  ...
  def get_topic_relation_score_equal(self, antecedent_cosin, antecedent_scores, span_cosin, span_score, dim2, dim3):
    ...
    # pre_topic_pre_mask = tf.logical_and(pre_topic_pre_mask, cosin_mask)
    #pre_topic_as_antecedent = tf.boolean_mask(pre_topic_pre_antecedent_relation, pre_topic_pre_mask)
    pre_topic_as_antecedent = tf.boolean_mask(antecedent_scores, pre_topic_pre_mask)
    return tf.reduce_sum(pre_topic_as_antecedent)
    # return tf.reduce_mean(pre_topic_as_antecedent)
