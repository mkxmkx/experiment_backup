...
//实现操作的内核。为操作提供实现，需是扩展OpKernel并替换Compute方法的类
...
public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    TTypes<int32>::ConstMatrix candidate_starts = context->input(1).matrix<int32>();     //一个span的起始位置
    ...
    TensorShape output_span_indices_shape({num_sentences, max_num_output_spans});  //output tensor维度：句子数目，最大输出span数目
    ...
    for (int i = 0; i < num_sentences; i++) {
      ...
      std::sort(sorted_input_span_indices[i].begin(), sorted_input_span_indices[i].end(),
                [&span_scores, &i](int j1, int j2) {
                  return span_scores(i, j2) < span_scores(i, j1);   //按照评分从小到大排序
                });
    }
    ...
    for (int l = 0; l < num_sentences; l++) {
      ...
      while (num_selected_spans < num_output_spans(l) && current_span_index < num_input_spans) {
        ...
        for (int j = start; j <= end; ++j) {
          auto latest_end_iter = start_to_latest_end.find(j);   //疑问？这步获得了什么，不太懂
        ...
        if (!any_crossing) {
          if (_sort_spans) {  //_sort_spans变量是表明该span是否排序过，bool值
            top_span_indices.push_back(i); //将i加到vector最后面
          } else {
